...
for ayear in [2009]:
	print ayear
	outfile = open('analysis_data/timeofday_' + str(ayear) + '.csv','w')
	...
	for weekday in range(0,7):
		for daytime in range(24):
			...
			c.execute("""SELECT ARR_DELAY, COUNT(ARR_DELAY) FROM (SELECT ARR_DELAY,CRS_DEP_TIME FROM flights WHERE YEAR = %s AND WEEKDAY = %s) as YEARDAY WHERE CRS_DEP_TIME >= %s AND CRS_DEP_TIME <= %s GROUP BY ARR_DELAY""",(ayear,weekday,time_bin.strftime('%H:%M:%S'),(time_bin + timedelta(minutes=59)).strftime('%H:%M:%S'))) # 
			'''
			...
			if total_flights == 0:
				print weekday,time_bin.strftime('%H:%M:%S'),total_flights,0,0,0,0,0,0,0
				outfile.write('%i,%s,%i,%.4f,%i,%.4f,%.4f,%.4f,%.4f,%i\n'%(weekday,time_bin.strftime('%H:%M:%S'),total_flights,0,0,0,0,0,0,0))
			else:
				print weekday,time_bin.strftime('%H:%M:%S'),total_flights,np.average(delay,weights=delay_count),median(delay,delay_count),\
					above(delay,delay_count,0),above(delay,delay_count,15),above(delay,delay_count,30),above(delay,delay_count,60),np.sum(delay_count)
